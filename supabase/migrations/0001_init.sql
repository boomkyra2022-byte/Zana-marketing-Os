-- ZANA Marketing OS — initial schema + RLS
...
-- ============================================================
-- profiles auto-provisioning: create a profile row whenever a
-- new auth.users row appears (Supabase Auth signup).
-- ============================================================
...
-- Helper: current user's role, read from profiles. SECURITY DEFINER so it
-- can be called from any policy without recursive RLS checks on profiles.
...
-- profiles: everyone authenticated can read all profiles (needed for
-- assignee/owner display); a user can update only their own row unless
-- they are admin/owner.
...
-- Business data: any authenticated user can read. Viewers cannot write.
-- Admin/owner/content_lead/creator/editor/media_buyer can write.
