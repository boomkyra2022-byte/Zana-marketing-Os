...
const KNOWLEDGE_TYPES = ['PRODUCT', 'PERSONA', 'BRAND', 'CONTENT_RULES', 'WINNING_CREATIVE', 'LEARNINGS'] as const;
...
function knowledgePayload(formData: FormData) {
  const type = str(formData, 'type') ?? 'LEARNINGS';
  return {
    title: str(formData, 'title') ?? '',
    type: (KNOWLEDGE_TYPES as readonly string[]).includes(type) ? type : 'LEARNINGS',
...
export async function createKnowledgeItem(formData: FormData) {
  ...
  if (error) redirect('/knowledge/new?error=' + encodeURIComponent(error.message));
...
export async function updateKnowledgeItem(id: string, formData: FormData) {
  ...
  if (error) redirect(`/knowledge/${id}?error=` + encodeURIComponent(error.message));
