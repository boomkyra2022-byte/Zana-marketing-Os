...
export async function createPersona(formData: FormData) {
  ...
  if (!payload.name) redirect('/personas/new?error=' + encodeURIComponent('Name is required'));

  const { error } = await supabase.from('personas').insert(payload);
  if (error) redirect('/personas/new?error=' + encodeURIComponent(error.message));
...
export async function updatePersona(id: string, formData: FormData) {
  ...
  if (error) redirect(`/personas/${id}?error=` + encodeURIComponent(error.message));
